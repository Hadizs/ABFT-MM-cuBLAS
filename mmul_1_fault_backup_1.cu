...
int main(int argc, char ** argv) {
	...
	float *A_checksum= (float *)malloc((nr_rows_A+1) * (nr_cols_A+1) * sizeof(float));
	    for (unsigned int i=0; i < nr_rows_C * nr_cols_C; i++) { A_checksum[i] = 0; }

	float *B_checksum= (float *)malloc((nr_rows_B+1) * (nr_cols_B+1) * sizeof(float));
	    for (unsigned int i=0; i < (nr_rows_C+1) * (nr_cols_C+1); i++) { B_checksum[i] = 0; }

	float *C_checksum= (float *)malloc((nr_rows_C+1) * (nr_cols_C+1) * sizeof(float));
	    for (unsigned int i=0; i < (nr_rows_C+1) * (nr_cols_C+1); i++) { C_checksum[i] = 0; }
	...
	build_checksum(A_row_checksum, A_col_checksum, A_checksum, h_A, nr_rows_A, nr_cols_A);//just for test
	build_checksum(B_row_checksum, B_col_checksum, B_checksum, h_B, nr_rows_B, nr_cols_B);//just for test
	...
	print_matrix(A_checksum, nr_rows_A+1, nr_cols_A+1);//for test
	...
	print_matrix(B_checksum, nr_cols_B+1, nr_cols_B+1);//for test
